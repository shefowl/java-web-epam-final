...
public class PostBusy implements Command {
    private static final Logger logger = LogManager.getLogger(PostBusy.class);
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            driverService.setBusy((Integer) session.getAttribute(SessionAttribute.USER_ID_ATTRIBUTE));
            request.setAttribute(RequestAttribute.BUSY, true);
            request.getRequestDispatcher(Page.DRIVER_BUSY).forward(request, response);
        }catch (ServiceException e){
            logger.error("error during command PostBusy", e);

...
public class OrderCrudRepositoryImpl extends AbstractRepository implements OrderCrudRepository {

    private final static Logger logger = LogManager.getLogger(OrderCrudRepositoryImpl.class);
    ...
    public static final String SQL_ORDER_REQUEST_COMPLETED_BY_USER_ID = "SELECT * FROM (SELECT * FROM carorder " +
            "WHERE userId=?) AS T WHERE completed=1";
    public static final String SQL_ORDER_REQUEST_NOT_COMPLETED_BY_USER_ID = "SELECT * FROM (SELECT * FROM carorder " +
            "WHERE userId=?) AS T WHERE completed=0";
    ...
    public static final String SQL_ORDER_DELETE_BY_ID =
            "SET FOREIGN_KEY_CHECKS=0; DELETE FROM carOrder WHERE id=?; SET FOREIGN_KEY_CHECKS=1";
    ...
    public List<Order> getByUserId(Integer userId) throws DaoException {
        return getAllByCondition(userId, SQL_ORDER_REQUEST_BY_USER_ID);
    }

    @Override
    public List<Order> getCompletedByUserId(Integer userId) throws DaoException {
        return getAllByCondition(userId, SQL_ORDER_REQUEST_COMPLETED_BY_USER_ID);
    }

    @Override
    public List<Order> getCurrentFromDriverList(Integer driverId) throws DaoException {
        return getAllByCondition(driverId, SQL_ORDER_REQUEST_FROM_DRIVER_LIST);
    }

    private List<Order> getAllByCondition(Integer id, String sqlRequest) throws DaoException{
        List<Order> orders = new ArrayList<>();
        try (Connection connection = ConnectionPool.getInstance().getConnection();
    ...
    public Order getCurrentByUserId(Integer userId) throws DaoException {
        Order order = null;
        try (Connection connection = ConnectionPool.getInstance().getConnection();
             PreparedStatement statement  = connection.prepareStatement(SQL_ORDER_REQUEST_NOT_COMPLETED_BY_USER_ID)) {
    ...
    public void setAccepted(Integer driverId, Integer orderId) throws DaoException {
        setByTwoIntegers(driverId, orderId, SQL_ORDER_UPDATE_ACCEPTED_BY_ID);
    }

    @Override
    public void setStartedById(Integer orderId, boolean started) throws DaoException {
        setBooleanById(started, orderId, SQL_ORDER_UPDATE_STARTED_BY_ID);
    }

    @Override
    public void setCompleted(boolean completed, Integer orderId) throws DaoException {
        setBooleanById(completed, orderId, SQL_ORDER_UPDATE_COMPLETED_BY_ID);
    }
    ...
    public void delete(Integer id) throws DaoException {
        deleteById(id, SQL_ORDER_DELETE_BY_ID);
    }

    @Override
    public void deleteFromDriverList(Integer orderId) throws DaoException{
        deleteById(orderId, SQL_DRIVER_ORDER_LIST_DELETE_BY_ORDER_ID);
    }
    @Override
    public void clearDriverOrderListExceptAccepted(Integer orderId) throws DaoException{
        deleteById(orderId, DELETE_FROM_DRIVER_ORDER_LIST_EXCEPT_ORDER_ID);
    }
